        ...
        if not response:
            ...
            if "-" in last_message_id:
                ...
                last_message_date = datetime.utcfromtimestamp(
                    int(last_message_timestamp) // 1000
                )
...
def averages_consumer_func():
    ...
    if h:
        last_message_id = h["last_message_id"]  # type: ignore
...
def main():
    ...
    if len(sys.argv) == 2:
        ...
        if not h:
            ...
            current_stream_key = h["current_stream_key"]  # type: ignore
            last_message_id = h["last_message_id"]  # type: ignore
            current_hourly_total = int(h["current_hourly_total"])  # type: ignore
            current_hourly_count = int(h["current_hourly_count"])  # type: ignore
